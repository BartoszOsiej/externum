...
_KNOWN_BASE = {"Int", "Float", "Str", "Bool", "Void", "Any", "Ptr", "Result", "Option", "Some", "None", "Ok", "Err"}
_BUILTIN_RETURNS = {
    "len": "Int",
    "str": "Str",
    "int": "Int",
...
def _base_of(t: str | None) -> str | None:
...
def _is_ptr(t: str | None) -> bool:
    return _base_of(t) == "Ptr"


def _is_copy(t: str | None) -> bool:
    if not t:
...
class _Sym:
    __slots__ = ("declared", "line", "mutable", "type")

    def __init__(self, type_: str | None, declared: bool, line: int, mutable: bool = False):
        self.type = type_
...
class TypeChecker:
    ...
    def _declare(self, name: str, type_: str | None, line: int, mutable: bool = False):
        self.symbols[name] = _Sym(type_, True, line, mutable)
        self._scope[-1].add(name)
        if _is_ptr(type_):
            self.pointers.add(name)

    def _lookup(self, name: str, line: int) -> _Sym | None:
        ...
        for (tname, cname), methods in self.impls.items():
            ...
            for mname, _p, ret in methods:
                ...
                if want and ret and _base_of(ret) != _base_of(want):
                    self._err(f"impl `{tname} for {cname}` method `{mname}` returns `{ret}`, trait requires `{want}`")
    ...
    def _check_node(self, node: ASTNode):
        ...
        if t == "UNSAFE":
            ...
            self._declare(
                node.value, node.children[0].value if node.children else "Any", 0, mutable=node.value in self.mutable
            )
        elif t == "FUNCTION":
            ...
            if node.children:
                ...
                if self._current_ret and self._in_unsafe == 0:
                    ...
                    if vtype and not self._compatible(vtype, self._current_ret):
                        self._err(f"return type `{vtype}` does not match declared `{self._current_ret}`", 0)
        elif t == "YIELD" or t == "RAISE":
            if node.children:
                self._check_expr(node.children[0])
        elif t == "ASSERT" or t == "DEL":
            for child in node.children:
                self._check_expr(child)
        elif t in ("CALL", "DEREF") or t == "EXPRESSION":
    ...
    def _check_function(self, node: ASTNode):
        ...
        for pname, ptype in ptypes.items():
            ...
            if self._in_unsafe == 0 and ptype is None and clean != "self" and not pname.startswith("*"):
    ...
    def _deref_target_name(self, node: ASTNode) -> str | None:
        if node.children and node.children[0].type == "IDENTIFIER":
            return node.children[0].value
        return None

    def _assign_target(self, name: str, type_: str | None, value: ASTNode, line: int):
        ...
        if existing is None:
            if self._in_unsafe > 0:
                ...
                if declared_type or name in self.globals:
                    self._declare(
                        name, declared_type or "Any", line, mutable=name in self.mutable or name in self.globals
                    )
                    existing = self.symbols.get(name)
                else:
            ...
            if self._in_unsafe == 0 and not existing.mutable and name not in self.globals:
                self._err(f"cannot reassign immutable binding `{name}` — declare `mut {name}: ...`", line)
                return
        if value is not None and existing.type and self._in_unsafe == 0:
            ...
            if vtype and not self._compatible(vtype, existing.type):
                self._err(f"cannot assign `{vtype}` to `{name}: {existing.type}`", line)
        ...
        if value is not None and value.type == "IDENTIFIER" and value.value != name:
            ...
            if (
                src is not None
                and not _is_copy(src.type)
                and value.value not in self.moved
                and value.value not in self.globals
                and self._in_unsafe == 0
            ):
    ...
    def _check_aug_assign(self, node: ASTNode):
        if node.children and node.children[0].type == "IDENTIFIER":
            ...
            if existing is not None and self._in_unsafe == 0 and not existing.mutable and target not in self.globals:
                self._err(f"cannot mutate immutable binding `{target}` — declare `mut {target}: ...`", 0)
            else:
        ...
        if fn in self.signatures and self._in_unsafe == 0:
            for arg in node.children:
                if arg.type == "IDENTIFIER" and arg.value not in self.globals:
                    ...
                    if src is not None and not _is_copy(src.type) and arg.value not in self.moved:
    ...
    # --------------------------------------------------------------- inference
    def _infer(self, node: ASTNode) -> str | None:
    ...
    def _infer_str(self, s: str) -> str | None:
        s = s.strip()
        if not s:
            return None
