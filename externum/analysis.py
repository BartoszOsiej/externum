...
def _expand(source: str, name: str, macro: dict) -> str:
    ...
    for m in pattern.finditer(source):
        ...
        if len(args) != len(params):
            raise MacroError(f"macro `{name}` expects {len(params)} argument(s), got {len(args)}")
        expanded = body
...
def check(
    ast: list[ASTNode], annotations: dict, signatures: dict, traits: dict, impls: dict, mutable: set[str] = None
) -> list[str]:
...
def check_or_raise(
    ...
    if errors:
        raise ExternumTypeError("Externum: " + "; ".join(errors))
